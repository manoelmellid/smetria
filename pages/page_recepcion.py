...
if st.session_state.logged_in:
    ...
    # Cargar los datos solo si no están ya cargados en session_state
    if 'df' not in st.session_state:
        st.session_state.df = git.cargar_datos(columnas_necesarias=['id', 'estado', 'fecha', 'nombre', 'email', 'latitud', 'longitud', 'tipo_incidencia', 'comentario'])
    ...
    if st.button("Marcar como Solucionado"):
        ...
        if id_input:
            if id_input in st.session_state.df['id'].values:  # Buscar el ID directamente en la columna 'id'
                # Actualizar el estado de la incidencia
                st.session_state.df.loc[st.session_state.df['id'] == id_input, 'estado'] = 'Solucionado'
    ...
    # Mostrar la tabla actualizada (sin recargar desde cero)
    st.dataframe(st.session_state.df)
